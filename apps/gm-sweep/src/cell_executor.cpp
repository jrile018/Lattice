...
Result<toml::table> override_config_with_cell_params(
    ...
    // Deep-copy the base config so we do not modify the original.
    toml::table result = base_config;

    for (const auto& [dotted_key, value_str] : cell.param_overrides) {
        // Parse dotted path: "signals.z_entry" -> ["signals", "z_entry"]
        std::vector<std::string> path_parts;
        {
            ...
            while (std::getline(iss, part, .)) {
                if (part.empty()) {
                    return gm::Error{gm::ErrorCode::kInvalidArgument,
        ...
        if (path_parts.empty()) {
            return gm::Error{gm::ErrorCode::kInvalidArgument, "empty dotted path"};
        }

        // Navigate to parent table, creating intermediate nodes as needed.
        ...
        // Set final key to the parsed value.
        ...
        try {
            ...
            return gm::Error{gm::ErrorCode::kParseFailure,
...
Result<fs::path> create_cell_directory(const fs::path& sweep_root, int cell_id) {
    ...
    try {
        ...
        return gm::Error{gm::ErrorCode::kIoFailure,
...
Result<double> extract_sharpe_from_backtest_results(const fs::path& backtest_results_path) {
    if (!fs::exists(backtest_results_path)) {
        return gm::Error{gm::ErrorCode::kNotFound,
    ...
    try {
        ...
        if (!results.contains("sharpe_ratio_daily")) {
            return gm::Error{gm::ErrorCode::kValidationFailure,
