...
// Quote s for shell safety (from gm-run pattern).
...
// Extract exit code from std::system() result.
...
CellResult execute_cell(
    ...
    // Step 2: Extract pipeline section and override with cell parameters.
    ...
    // Step 4: Symlink upstream artifacts (gm-universe through gm-boundaries).
    ...
    // Step 5: Write per-cell config (pipeline section only - stages will read it).
...
";
    ...
    // Step 6: Run gm-signals (reads from upstream artifacts, outputs spreads/baskets).
    {
        ...
        spdlog::debug("Cell {}: running gm-signals: {}", cell.cell_id, cmd.str());
    ...
    // Step 7: Run gm-backtest (reads from signals and upstream artifacts).
    {
        ...
        spdlog::debug("Cell {}: running gm-backtest: {}", cell.cell_id, cmd.str());
        int raw_exit = std::system(cmd.str().c_str());
        int exit_code = normalized_exit_code(raw_exit);
    ...
    // Step 8: Extract Sharpe ratio from backtest_results.json.
